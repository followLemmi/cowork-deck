...
- [ ] `/clear` twice in one session, then restart the app — the tile is in the **third** conversation, not the second. Do the second clear and quit **immediately**, before the next poll tick, and it still holds: the tick is not the only thing that writes the id down. Same again with the window unfocused between the clear and the quit — since #251 that window never polls, so this is the case with no tick at all.
