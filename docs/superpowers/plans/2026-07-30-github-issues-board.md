...
- **The clippy ceiling is 6 warnings**, carried from the two previous plans: `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets` reports exactly 4 × `std::io::Error::other` and 2 × `too_many_arguments` (one of them `start_session`, already at 10/7). Run it **without** `-D warnings`.
  ...
  **But that correction does not transfer to Task 7, and the sentence that used to follow it here — "the count is 6 at every task" — was wrong** (corrected 2026-07-30 while executing Task 7). The reason it held for Tasks 5–6 is that `src-tauri/src/tasks/*` is reachable from `lib.rs:7`, `pub mod tasks`, so those items are public items of a *library*. `tasks_cmd.rs` is not: it is declared only at `main.rs:12`, `mod tasks_cmd;`, so it belongs to the **binary** crate alone, and in a bin crate `dead_code` fires on `pub` items normally. **`TrackerKind` therefore trips `dead_code` until something outside the test module reads it, and the expected count is not 6 for three tasks running:**
  ...
  Do **not** paper over the intermediate 7 with `#[allow(dead_code)]`: it is a suppression this plan forbids, it would have to be taken out again three tasks later, and it hides the signal that says whether Task 10 actually wired the field. **Take the expected number from the three bullets above, not from a restatement here** — this sentence used to carry its own copy of the counts, drifted out of step with the bullets when Task 9's was corrected from 7 to 8, and was the line an agent skims. Anything above the bullet for the task you are on, or anything still above 6 after Task 10, is a real new warning.
...
- "`ipc.ts`'s `TrackerConfig` types `type` as the literal `"fs"`" — **true, and it is the only one of the four that was ever load-bearing.** Verified at `ipc.ts:231`.

**So the barrier rests on one fact, not four:** no stored workspace can have a GitHub tracker, because the only thing that writes a tracker config is the workspace form, and `ipc.ts:231` types the provider as the literal `"fs"`. Every GitHub branch added in Phase 2 is therefore reachable only from tests — not because the branches do not exist, but because no workspace can be in the state that selects them. Task 24 is what opens that door, and Task 18 is what widens the type. **Verify the barrier by checking that one fact**, plus the full `cargo test` and the clippy count at exactly 6; do not work the old four-point list, which would report a violation that is not one.

**One correction to how this used to be phrased** (2026-07-30): "nothing user-visible has changed" was overstated, and not because of anything GitHub. Tasks 1–2 changed what happens on a **damaged store file**: an unparseable `workspaces.json` or `skills.json` used to be swallowed into an empty list and written back over the user's data, and now returns `InvalidData` which reaches the UI through `commands.rs:110`. So for a user whose store file is damaged, saving now visibly fails where it previously appeared to succeed. That is #117's fix working as designed and a strict improvement — but it is user-observable, it needs no frontend change, and a barrier that claimed otherwise would have been claiming something false about the one commit pair most likely to surprise someone. |
| 3 — Frontend: types, pure rules, the prompt, the view, the wiring, the form | 18–24 | The full gate. The feature is usable end to end. **Barrier C's manual check needs two GitHub workspaces whose repositories share an open issue number** — added 2026-07-31 from the review. With one workspace, the defect that review found is invisible: `Deck.taskLinks()` carried no workspace dimension, which was correct while card ids were globally-unique ULIDs and became wrong the moment a source arrived whose ids are only *repository*-unique. Fixed in `177ce33`; a one-workspace check cannot see either the bug or the fix. **It is check 20 of Task 26's list, which cites this row in turn** — the requirement lived only here until 2026-07-31 and was therefore owed by nobody, so the two now point at each other and neither can be deleted without the other reading as wrong. |
...
Expected: PASS, the existing `tasks_cmd` tests plus 7 — Step 1 lists seven test functions. (Corrected from "plus 6" on 2026-07-30: measured 54 → 61.)
...
(Corrected 2026-07-30: this line said `GITHUB_BOARD`, a constant, while Step 1's test and Step 3's implementation both use a function. A `const` is impossible anyway — `BoardConfig` owns `Vec`s and `String`s. Follow the code.)
...
**And one of the six refusals was narrowed afterwards, deliberately** — `fix(issues): only a github board refuses a migration question`, authorised 2026-07-30. `tasks_migration_status` keeps a GitHub-only guard (`migration_refusal`, a pure function beside the other rules, because the command takes `State` and no unit test can construct one) and returns `Ok(None)` again for an unconfigured workspace and for a #117 `Unknown` source.

Two reasons, and the second is the one that made it worth a commit rather than a note:
- The refusal corrected nothing for a GitHub workspace. `offer_for` already returned `Ok(None)` for one at `:716`, so the check converted a correct silent answer into an error — and did so for *unconfigured* workspaces too, on every board tick, which is the common case and not the one being narrowed.
- **The only thing hiding that was one `catch (e) { console.debug(…) }` at `main.ts:247-248`, and Tasks 21–22 rewrite that chain.** In the same function `listTasks` sets `error = String(e)` while this call swallows. An agent doing Task 22 who tidies that asymmetry — a reasonable thing to do — makes every workspace with no tracker display a refusal sentence, from a change made three tasks earlier that they have no reason to inspect.

**`tasks_migration_dismiss` deliberately keeps the full `fs_provider_for` check.** Consistency argues for narrowing it too and is outweighed: it has one caller, `main.ts:365`, a button press, so it is not polled and has no swallowing catch for Task 22 to rewrite — neither justification above applies. The line falls where it actually is: `status` is the only *polled read* of the six, and the other five are person-triggered actions where "not-configured" genuinely was the wrong sentence. There is a comment on `dismiss` saying so, because two neighbours doing different things is otherwise something the next reader will "fix".
...
Run: `cd src-tauri && cargo test commands`
Expected: PASS, plus 4. The **four** existing `session_env` tests must be updated with `None, None` and must keep asserting exactly what they asserted before — if one of them needs a *changed* assertion, the branch is leaking.

(Corrected 2026-07-30: this said "two". There are four callers in `commands.rs` — `session_env_carries_tracker_paths_when_configured`, `session_env_omits_tracker_vars_when_not_configured`, `a_session_launched_from_a_card_carries_its_id`, `a_session_launched_without_a_card_carries_no_card_id`. Measured: all four took `None, None` and none needed a changed assertion, so the invariant held.)
...
Expected: FAIL — but **3 of the 4, for two different reasons.** (Corrected 2026-07-30, measured while executing; this line used to claim all four fail on empty stdout, which would have left an executing agent unable to tell a plan error from their own.)

- Two rows fail as described: the current `guard` returns 0 with no output when `COWORK_TASKS_DIR` is unset, so their announcement assertions fail on empty stdout.
- **`github_guard_wins_when_a_tracker_directory_is_also_set` fails differently**, because it sets `COWORK_TASKS_DIR` deliberately — that is the entire point of the row. So it reaches the existing file no-card announcement branch and fails on `the github branch must never name a folder`, not on empty stdout.
- **The `Stop` row passes before the change, vacuously**, since the old `guard` returns 0 with no output when the directory is unset. A vacuous pass is expected here and is not a sign the test is wrong; it becomes meaningful once the branch exists.
...
Expected: PASS, **24 + 4 = 28** — four `#[test]` functions are added. (Corrected 2026-07-30: this said 24 + 5. The parenthetical was right that the `for` loop in the `Stop` test counts as one test and the arithmetic then added five anyway. Measured: `--test cowork_task` is 28.)

**Add `.env_remove("COWORK_ISSUE_REPO")` to the `guard` helper at `tests/cowork_task.rs:237`, unconditionally**, with a comment saying why. Once `guard()` dispatches on that variable *before* reading `COWORK_TASKS_DIR`, every file-tracker row in this file depends on it being unset — and children inherit the harness's environment, so `cargo test` run from a process that exports it sends ten-plus unrelated rows down the GitHub branch. That process is not hypothetical: **Task 12 is what exports it**, so it is any cowork-deck session in a GitHub-backed workspace. A suite that breaks when run from inside the thing it tests is a trap; the rows must state their own environment rather than inherit one. Landed as `fix(issues): the guard harness clears the variable that now chooses the branch`.
...
`/search/issues?q=…&per_page=1`. GraphQL `repository.issues.totalCount` is
...
None of the following is covered **end to end** by an automated test. Where a pure rule *is* pinned, the check says so and names what is left to a person — that seam is usually the half the fix depends on. Run it and record the result in the pull request description. `gh` against `followLemmi/cowork-deck` needs `GH_TOKEN=$(gh auth token --user followLemmi)` — the default account here is an EMU that cannot write there, and a plain call fails with a confusing permission error.

Two conventions, because they look contradictory and are not. **Assert properties, not UI strings**: a checklist that quotes interface text decays silently, since the next person to improve a sentence has no reason to suspect a document three directories away describes something that no longer exists. Where a string is quoted below it is an *illustration* of the shape to look for, not the thing being checked. **Check 11 is the deliberate exception** — its whole purpose is to record a string nobody has yet observed, so paraphrasing it would defeat the check. And references are by **commit SHA, not by review finding label**: both Barrier B and Barrier C numbered their findings from F1, so "F1" alone can name two different defects.
...
2. **The count line**, against a repository with more than 50 open issues. The property: the line names how many are shown *and* how many exist, and both match what the browser says — "Showing 50 of 63 open issues." is the shape that looks like today. **A capped page whose totals call failed claims only the number it can prove** (`ad0862e`) — that is the honest fallback and a pass, not a failure of this check. Then a repository with fewer than 50 open issues — no count line at all, and (watching the process list, or with a temporary log) **no totals call**.
...
4. **The file board, re-checked** — this is the check Task 22 earns, and nothing about the file board was asked for. **Narrower than when it was written:** `tests/pr-polling.test.ts` drives `main.ts`, so the poll chain between the two ends is pinned — once on entry, once per 5 s, silent on blur, re-armed on focus. What no test can reach is the ends. (a) Edit a card on disk from outside the app and confirm the board picks it up within five seconds — the watcher, and WebKitGTK delivering it. (b) Blur the window and confirm the board falls silent, then refocus and confirm it reads **on return** rather than waiting for the next tick — that is whether the real webview gets `focus`/`blur` at all. Confirm the sidebar counts still update after a card edit while you are in there; it costs one glance.
   **The file board falling silent on blur is deliberate, and is to be recorded as expected rather than filed.** It is the only behaviour change in this branch to a screen that shipped months ago: the poll used to run blind, and Task 22 gated it for a decision about a different screen. A tester meeting it cold reports it as a regression unless this says otherwise.
...
11. **One throwaway `gh issue close` against a scratch repository**, from a terminal, and **record what its success output actually is**. Nothing in this feature parses it — the board refetches — but nobody has seen it, and that should stop being true before the write path ships. Paste the output verbatim into the pull request description. The `create` half of this check is **done**: filing #117 on 2026-07-30 used the exact argv shape this feature builds (`--repo`, `--title`, `--body-file`) and printed the new issue's URL on stdout and nothing else, exit 0. So `--body-file` is confirmed to carry a multi-line body, and the number is recoverable from the output — which changes nothing, because the board still refetches.
   **The verbatim-paste instruction is deliberate — do not "fix" it into a property.** The preamble's rule is to assert properties rather than quote UI or tool strings, and this check is the exception *for the same reason the rule exists*: everywhere else a quoted string is a stale copy of something already known, whereas here the string is the deliverable. Nobody has observed this output, so a paraphrase would record what the runner expected instead of what `gh` said, which is the one thing this check is for.
12. **The rate-limit banner, driven by an injected low `X-Ratelimit-Remaining`** — not by provoking a real refusal, whose own text is unverified and which nothing matches on. Temporarily return a low value from `split_gh_response`, or point `COWORK_GH_PATH` at a wrapper script that rewrites the header, and confirm the banner appears and the board keeps its list.
   **Two conditions, and without them this check cannot pass at all.** It must run against **a repository with more than 50 open issues**, because the signal rides on the totals call and decision 7 fires that only on a capped page — on a scratch repository nothing happens, which reads as a broken feature rather than an unmet precondition. And the injected value must be **below 250** (`RATE_WARN_BELOW`, `src/issues.ts`); inject 300 and the correct behaviour is silence. Both were missing when this check was written, and either omission produces a false failure — worse than an untested feature, because it spends the tester's trust.
13. **The switch confirmation against a root that really holds cards.** Configure a folder tracker, file two cards, switch the workspace to GitHub issues: the confirmation must name the count and the **full old path**. Decline it — the form stays open. Accept it — the board switches, and the two card files are still on disk, untouched. Switch back and confirm both cards return.
   Then the negative half, one more form open: the confirmation fires **only when a file root is being left behind** (`src/forms.ts`), so a workspace with no tracker at all, and a workspace already on GitHub issues, must both switch **without** being asked. A confirmation that appears where there is nothing to abandon is a sentence about a folder that does not exist.
14. **A private repository.** Everything measured for the spec was measured against a public one, so nothing so far demonstrates that the per-workspace token is what makes the read path work. Bind an account with access, confirm the board lists the issues, then unbind it and confirm the board says `no-account` rather than showing a stale or empty list. The mechanism, so the expectation is precise: `board.ts` returns on `unavailable` **before** the last-good list is drawn, so the `no-account` screen is expected to *replace* the stale rows, not sit above them. Rows still on screen there are the finding.
15. **The three unavailable states**, each showing its own screen and — **for two of the three** — its own next step: a workspace with no bound account, a folder that is not a git repository, and (by pointing `COWORK_GH_PATH` at a nonexistent file) no `gh` at all. `no-repo` deliberately offers **no** button (`action: null`): nothing in the app turns a folder into a clone, and a dead button is worse than none. This item promised "its own next step" for all three until 2026-07-31, which was false for that state.
   **What to assert about the words is a property, not a string.** On the board, the sentence names *issues*, or names neither view, and **in no case names pull requests**; on the pull request list, the mirror of that. As of `a410aa3` both views take these three states from one neutral module (`src/gh-unavailable.ts`) which is handed its subject by the caller — so wrong vocabulary on either screen is now a regression in a **shared** constant rather than a defect in one view. Check the other screen before filing it. The defect this replaced was exactly that: the board imported the pull request view's copy and told a person with no `gh` that their *pull requests* could not be read.
...
17. **▶ on a same-repo PR whose branch was pushed from the workspace makes a *new* worktree.** Added 2026-07-31 after the Barrier B fix `9fa3ec6`, and the one difference in this round a person will actually notice: this used to hand back **the workspace root itself**, because `git worktree list --porcelain` lists the main working copy first and the reuse lookup returned the first match on the branch. So the session opened in the workspace, alongside every other session there. It now creates `<ws>-pr/<n>-<branch>` with `reused: false`. Check the tile says the new path and not the workspace path — this is a behaviour change, not only a bug removal, so it is worth confirming by eye rather than inferring from a green suite.
18. **A worktree you made yourself is left alone.** `git worktree add ../scratch <branch>` by hand, then ▶ on a PR for that branch: the app must build its own directory rather than adopting yours. Reuse exists for the worktree the issue flow created; adopting a hand-made one would tell the tile it is the same piece of work. **Put it where the path says — `../scratch`, and specifically *not* inside `<ws>-pr/` or `<ws>-issue/`.** Only the *parent* directory is compared, so a hand-made worktree sitting in one of the app's own sibling directories **would** be adopted; a tester who "simplifies" this by putting the scratch tree next to the app's own gets a pass that means the opposite of what it appears to.
19. **A session in a GitHub workspace, checked from the inside.** Start one plainly (no issue) and ask it to print its environment: no `COWORK_TASKS_DIR`, no `COWORK_PROJECT`, no `COWORK_TASK_BIN`, and `COWORK_ISSUE_REPO` set. Confirm the `UserPromptSubmit` context names the repository and `gh issue create`. Then let it finish — the `Stop` must not be blocked. **Run it online.** `COWORK_ISSUE_REPO` is set only when `repo_facts_for` resolved the repository, so offline *every* variable is unset: the absence half passes, the presence half is silently skipped, and the check reports success having tested half of itself. (This item was numbered 17 until 2026-07-31, colliding with the check above it — checks 17 and 18 were inserted for the worktree-reuse fix without renumbering it.)
20. **Two GitHub workspaces whose repositories share an open issue number.** Added 2026-07-31; required by phase 3's row in the table above, which is the *only* place the requirement existed until now. Fixed in `177ce33`: `Deck.taskLinks()` carried no workspace dimension, which was correct while every card id was a globally-unique ULID and became wrong the moment a source arrived whose ids are only *repository*-unique. With one workspace neither the bug nor the fix is visible.
   Two workspaces, A and B, each a GitHub board whose repository has an open **#42**. Start a session on A's #42. Then on B's board: #42 must **not** read "in progress", ▶ must be offered on it, and ▶ must open a session in **B's own** worktree rather than focusing A's tile.
   **Run it in both orders — start from A, then repeat starting from B.** The defect was asymmetric in effect: a *busy* session on A's #42 withheld ▶ on B's #42 entirely, while a merely *idle* one offered ▶, really created a worktree in B, and then focused A's tile — a terminal attached to the wrong repository with a stray worktree left behind. Both halves come from the same missing dimension but present as different bugs, so a tester who only ever starts from A sees one and assumes the other is fine.
   The pure rule is pinned (`tests/tasks.test.ts:262`). **What is manual is whether a real tile carries its `workspaceId` at all** — which is the half the fix depends on.
21. **Editing an issue from the card.** Added 2026-07-31; nothing on this list exercised `update`'s edit path, and it is the sharpest write in the feature. `gh issue edit` writes title **and** body every time — the title on argv, the body on stdin — so a title-only patch has to resend the body it was not given, and `computePatch` really does send a rename as `title` alone. Before `3f69795` that blanked the body of a public issue.
   Rename an issue whose body is **multi-line and contains a backtick and a `$`** — the fixture is deliberate, it is what catches a shell-quoting regression — save from the card, and confirm on GitHub that the body is **byte-identical**, not merely that it looks the same. Then edit the body alone and confirm the title is unchanged. The argv is pinned by tests; nothing has ever run this against GitHub.
22. **(Optional) The board for a source this build cannot read.** The forward half of #117, and the only thing the README's downgrade warning actually promises a reader. Hand-edit a workspace's record in `workspaces.json` to `{"type":"jira"}`, launch, and confirm three things: the workspace keeps its **sidebar row**, its board says the source "was saved by a newer version of the app, or is damaged, and cannot be read here. Nothing has been changed.", and saving that workspace from the form leaves the record **byte-identical**.
   Optional because running it means editing the installed store — the same hazard that keeps this whole list off any automated runner. Rust tests already cover the parse and the round-trip; what is unverified is only the sidebar row and the board sentence.
   **Take a copy of `workspaces.json` first — and note that this is not the thing gap 1 forbids.** Gap 1 forbids the *README* offering a backup as a **user's safeguard against a defect we shipped**, because that hands our problem to them and calls it advice. A developer copying their own store before deliberately corrupting it to run a test is a different act with a different subject: the risk is one they are creating on purpose, for themselves, in the next five minutes. The two sentences will eventually be read together; the reason is here so they are reconciled correctly rather than by deleting one.
...
**Six gaps. Five were found in the spec while planning against it; the sixth was found while executing Task 12 and is a coverage gap in this plan rather than in the spec.** None is a decision reopened; each is a place a mechanism was left unspecified, specified in a way that cannot work as written, or left unproven.
...
6. **`start_session`'s GitHub branch has no test, and the leak invariant is asserted one level below where it can break.** Found while executing Task 12 (2026-07-30), and not fixable inside that task as written. `start_session` is a `#[tauri::command]` taking `State` and `AppHandle`, so nothing in the suite constructs it; all four of Task 12's tests call `session_env` directly. The invariant the fourth test's doc comment leans on — "`root` is `None` exactly when the tracker is GitHub" — is therefore asserted about **`session_env`'s parameters**, not about the code that fills them. So `session_env` is proven not to leak `COWORK_TASKS_DIR` when handed `None`, and nothing proves the `TrackerKind::GitHub` arm hands it `None`: an edit that passed a root from that arm would leak the variable into a GitHub session **with every test still green.** The leak test itself is sound — it was verified by mutation, not by reading, and it does fail with `COWORK_TASKS_DIR must not be set for a github workspace` when `session_env` is made to push it. The gap is the seam above it. Whether closing it needs a small extracted helper that a test can reach was a **Barrier B** decision — line 3736, the checkpoint after Task 17. (This said "Barrier A" when first written, which would have filed a live decision against a checkpoint already passed, so it would never have been taken. Caught on the Task 17 report, 2026-07-30.)

**Decided at Barrier C, 2026-07-31: no extraction, and this gap overstates its own risk.** `TrackerKind::GitHub` is a **fieldless variant** — inside that arm there is no `root` in scope at all. Leaking `COWORK_TASKS_DIR` into a GitHub session therefore requires *fabricating* a path (`Some(PathBuf::from(&ws.path))`), not misplacing an existing binding, which is a materially different class of mistake from the one described above. **The invariant is structural, not conventional**, and this paragraph should not be read as saying otherwise.

Against that, every extraction shape costs something real: injecting `repo_facts_for` drags `State<AppState>` or a closure into the helper, and returning only the root splits one `match` on `TrackerKind` into two — and two matches on the same enum is a *new* way for the seam to disagree with itself, which is the exact failure mode this plan closes elsewhere. If the invariant is ever to be made unrepresentable rather than merely unlikely, the shape that earns its keep is a pure `fn session_tracker(ws) -> SessionTracker` whose `Repo` variant **has no root field**, with both side effects staying at the one call site that matches it. That is a type rather than a test, and strictly better than the test this gap asks for. Polish, not a barrier item.

One thing checked while ruling on it, since Phase 3 made the path reachable: when `repo_facts_for` fails (offline), `issue_repo` is `None`, so `COWORK_ISSUE_REPO` and `COWORK_ISSUE_NUMBER` are both unset, and `guard` (`bin/cowork_task.rs:202-215`) falls through to a `COWORK_TASKS_DIR` that is also unset and returns 0. Silent, and no false sentence — correct degradation.
