...
18. **A worktree you made yourself is left alone.** `git worktree add ../scratch <branch>` by hand, then ▶ on a PR for that branch: the app must build its own directory rather than adopting yours. Reuse exists for the worktree the issue flow created; adopting a hand-made one would tell the tile it is the same piece of work.
