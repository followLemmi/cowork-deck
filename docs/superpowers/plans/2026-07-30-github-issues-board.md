...
- **The clippy ceiling is 6 warnings**, carried from the two previous plans: `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets` reports exactly 4 × `std::io::Error::other` and 2 × `too_many_arguments` (one of them `start_session`, already at 10/7). Run it **without** `-D warnings`.
  **Count diagnostics, not lines.** `grep -c '^warning'` reports **8** at the ceiling, because cargo prints a per-crate summary line ("warning: `cowork-deck` generated N warnings") that is not itself a diagnostic. Use `grep -c '^warning: ' | ` minus the summaries, or simply read the two summary lines and add them — they are the authority. An agent counting with a bare grep will believe it is two over before it has changed anything. A task may raise the test counts; it may not raise the warning count. **One sanctioned exception:** Tasks 5–6 sit up to two warnings above it, because `issue_branch`, `issue_worktree_path` and the argv builders land there and nothing calls them until Tasks 10 and 14. Task 14 must bring the count back to exactly 6; if it does not, something it was supposed to wire up is missing.
...
The three commands behind ▶ on an issue and the cleanup offer after it closes. This task also brings the clippy count back to exactly 6: `issue_branch` and `issue_worktree_path` finally have callers.
