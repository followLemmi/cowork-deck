...
| 2 — Rust seams: the IPC layer, the session environment, the sidecar, the worktrees | 7–17 | Full `cargo test`, plus the clippy count back at exactly 6. The backend is complete and exercised only by tests; the frontend still cannot produce a GitHub tracker config, so **no new user-reachable feature exists yet** — verified at Barrier A on every point: `GhIssueProvider` is never constructed outside its own tests, no Tauri command was added or registered, `provider_for` still returns a concrete `FsTaskProvider` so there is no seam to arrive through, and `ipc.ts`'s `TrackerConfig` types `type` as the literal `"fs"`, so the form cannot author one. That is deliberate — it is the last point at which a backend bug is unambiguously a backend bug.
