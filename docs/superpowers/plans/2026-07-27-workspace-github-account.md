...
# ФАЗА 1 — аутентификация и настройка
...
# ФАЗА 2 — PR и issues воркспейса

### Task 14: Запросы к GitHub через `gh`

**Files:**
- Modify: `src-tauri/src/gh.rs`

**Interfaces:**
- Consumes: `gh::session_env` (Task 3), `gh::which_gh` (Task 2)
- Produces: `gh::PrItem { number: u64, title: String, author: String, is_draft: bool, review_decision: Option<String>, updated_at: String, url: String }`, `gh::IssueItem { number: u64, title: String, author: String, labels: Vec<String>, updated_at: String, url: String }`, `gh::parse_prs(json: &str) -> Vec<PrItem>`, `gh::parse_issues(json: &str) -> Vec<IssueItem>`, `gh::repo(cwd: &str, env: &[(String, String)]) -> Option<String>`, `gh::pull_requests(cwd, env) -> Result<Vec<PrItem>, String>`, `gh::issues(cwd, env) -> Result<Vec<IssueItem>, String>`

- [ ] **Step 1: Write the failing tests**

В `mod tests` в `src-tauri/src/gh.rs`:

```rust
    #[test]
    fn parses_pull_request_list() {
        let json = r#"[
          {"number":14,"title":"feat: scheduled scenarios","author":{"login":"followLemmi"},
           "isDraft":false,"reviewDecision":"APPROVED","updatedAt":"2026-07-27T10:00:00Z",
           "url":"https://github.com/o/r/pull/14"},
          {"number":15,"title":"wip","author":{"login":"other"},"isDraft":true,
           "reviewDecision":"","updatedAt":"2026-07-26T10:00:00Z","url":"https://github.com/o/r/pull/15"}
        ]"#;
        let prs = parse_prs(json);
        assert_eq!(prs.len(), 2);
        assert_eq!(prs[0].number, 14);
        assert_eq!(prs[0].author, "followLemmi");
        assert!(!prs[0].is_draft);
        assert_eq!(prs[0].review_decision.as_deref(), Some("APPROVED"));
        assert!(prs[1].is_draft);
        assert_eq!(prs[1].review_decision, None, "пустая строка — это отсутствие решения");
    }

    #[test]
    fn parses_issue_list_with_labels() {
        let json = r#"[{"number":9,"title":"schedules","author":{"login":"a"},
          "labels":[{"name":"feature"},{"name":"ui"}],
          "updatedAt":"2026-07-27T10:00:00Z","url":"https://github.com/o/r/issues/9"}]"#;
        let issues = parse_issues(json);
        assert_eq!(issues.len(), 1);
        assert_eq!(issues[0].labels, vec!["feature", "ui"]);
    }

    #[test]
    fn parses_empty_and_broken_lists_as_empty() {
        assert!(parse_prs("[]").is_empty());
        assert!(parse_prs("not json").is_empty());
        assert!(parse_issues("[]").is_empty());
        assert!(parse_issues("{\"unexpected\":true}").is_empty());
    }
```
...
Run: `cargo test --manifest-path src-tauri/Cargo.toml gh::`
Expected: FAIL — `cannot find function parse_prs`.

- [ ] **Step 3: Write minimal implementation**

В `src-tauri/src/gh.rs`:
...
fn author_of(v: &serde_json::Value) -> String {
    v.get("author").map(|a| str_at(a, "login")).unwrap_or_default()
}

pub fn parse_prs(json: &str) -> Vec<PrItem> {
    ...
    arr.iter()
        .map(|v| PrItem {
            ...
            review_decision: Some(str_at(v, "reviewDecision")).filter(|s| !s.is_empty()),
...
pub fn parse_issues(json: &str) -> Vec<IssueItem> {
    let arr: Vec<serde_json::Value> = serde_json::from_str(json).unwrap_or_default();
    arr.iter()
        .map(|v| IssueItem {
            number: v.get("number").and_then(|n| n.as_u64()).unwrap_or(0),
            ...
            labels: v
                ...
                .map(|a| a.iter().map(|l| str_at(l, "name")).filter(|s| !s.is_empty()).collect())
                .unwrap_or_default(),
            updated_at: str_at(v, "updatedAt"),
            url: str_at(v, "url"),
        })
...
/// Запускает gh в каталоге воркспейса с его же окружением — поэтому запрос
/// уходит от лица аккаунта воркспейса, а не активного аккаунта gh.
fn run(cwd: &str, env: &[(String, String)], args: &[&str]) -> Result<String, String> {
    let path = which_gh().ok_or_else(|| "gh не найден".to_string())?;
    let mut cmd = std::process::Command::new(path);
    cmd.args(args).current_dir(cwd);
    for (k, v) in env { cmd.env(k, v); }
    let out = cmd.output().map_err(|e| redact(&e.to_string()))?;
    if !out.status.success() {
        return Err(redact(String::from_utf8_lossy(&out.stderr).trim()));
    }
    Ok(String::from_utf8_lossy(&out.stdout).to_string())
}

pub fn repo(cwd: &str, env: &[(String, String)]) -> Option<String> {
    let out = run(cwd, env, &["repo", "view", "--json", "nameWithOwner"]).ok()?;
    let v: serde_json::Value = serde_json::from_str(&out).ok()?;
    v.get("nameWithOwner").and_then(|n| n.as_str()).map(|s| s.to_string())
}

pub fn pull_requests(cwd: &str, env: &[(String, String)]) -> Result<Vec<PrItem>, String> {
    let out = run(cwd, env, &[
        "pr", "list", "--state", "open", "--limit", "30",
        "--json", "number,title,author,isDraft,reviewDecision,updatedAt,url",
    ])?;
    Ok(parse_prs(&out))
}

pub fn issues(cwd: &str, env: &[(String, String)]) -> Result<Vec<IssueItem>, String> {
    let out = run(cwd, env, &[
...
```bash
git add src-tauri/src/gh.rs
git commit -m "feat(github): read PRs and issues through gh under the workspace account"
```

---

### Task 15: Команды PR/issues и кеш во фронте

**Files:**
- Modify: `src-tauri/src/commands.rs`, `src-tauri/src/main.rs`
- Modify: `src/ipc.ts`, `src/github.ts`, `tests/github.test.ts`

**Interfaces:**
- Consumes: `gh::pull_requests`, `gh::issues`, `gh::repo` (Task 14), `resolve_session_auth` (Task 6)
- Produces: Rust `gh_repo(state, workspace_id: String) -> Result<Option<String>, String>`, `gh_pull_requests(state, workspace_id: String) -> Result<Vec<gh::PrItem>, String>`, `gh_issues(state, workspace_id: String) -> Result<Vec<gh::IssueItem>, String>`; TS `PrItem`, `IssueItem`, `ghRepo`, `ghPullRequests`, `ghIssues`, и чистые `isStale(fetchedAtMs, nowMs, ttlMs?)`, `relativeAge(fetchedAtMs, nowMs)`, `sortPrs(items)`

- [ ] **Step 1: Write the failing tests**

В `tests/github.test.ts`:

```ts
import { isStale, relativeAge, sortPrs } from "../src/github";
import type { PrItem } from "../src/ipc";

const pr = (over: Partial<PrItem> = {}): PrItem => ({
  number: 1, title: "t", author: "a", isDraft: false, reviewDecision: null,
  updatedAt: "2026-07-27T10:00:00Z", url: "u", ...over,
});

describe("isStale", () => {
  it("expires after the TTL", () => {
    expect(isStale(1000, 1000 + 59_000)).toBe(false);
    expect(isStale(1000, 1000 + 61_000)).toBe(true);
    expect(isStale(1000, 1000 + 5_000, 1_000)).toBe(true);
  });
});

describe("relativeAge", () => {
  it("formats the age in Russian", () => {
    const base = Date.UTC(2026, 6, 27, 10, 0, 0);
    expect(relativeAge(base, base + 5_000)).toBe("обновлено только что");
    expect(relativeAge(base, base + 3 * 60_000)).toBe("обновлено 3 мин назад");
    expect(relativeAge(base, base + 2 * 3_600_000)).toBe("обновлено 2 ч назад");
  });
});

describe("sortPrs", () => {
  it("puts drafts last and sorts the rest by recency", () => {
    const items = [
      pr({ number: 1, isDraft: true, updatedAt: "2026-07-27T12:00:00Z" }),
      pr({ number: 2, updatedAt: "2026-07-25T10:00:00Z" }),
      pr({ number: 3, updatedAt: "2026-07-27T09:00:00Z" }),
    ];
    expect(sortPrs(items).map((p) => p.number)).toEqual([3, 2, 1]);
  });

  it("does not mutate the input", () => {
    const items = [pr({ number: 1, isDraft: true }), pr({ number: 2 })];
    sortPrs(items);
    expect(items.map((p) => p.number)).toEqual([1, 2]);
  });
});
```
...
Run: `npx vitest run tests/github.test.ts`
Expected: FAIL — `isStale is not exported`.
...
export interface PrItem {
  number: number; title: string; author: string; isDraft: boolean;
  reviewDecision: string | null; updatedAt: string; url: string;
}
export interface IssueItem {
  number: number; title: string; author: string; labels: string[];
  updatedAt: string; url: string;
}
export const ghRepo = (workspaceId: string) => invoke<string | null>("gh_repo", { workspaceId });
export const ghPullRequests = (workspaceId: string) => invoke<PrItem[]>("gh_pull_requests", { workspaceId });
export const ghIssues = (workspaceId: string) => invoke<IssueItem[]>("gh_issues", { workspaceId });
```

В `src/github.ts`:

```ts
import type { PrItem } from "./ipc";

const TTL_MS = 60_000;

/** Кеш живёт в памяти и обновляется по требованию — фонового опроса GitHub у
 *  нас нет сознательно (рейт-лимиты и батарея). */
export function isStale(fetchedAtMs: number, nowMs: number, ttlMs: number = TTL_MS): boolean {
  return nowMs - fetchedAtMs > ttlMs;
}

export function relativeAge(fetchedAtMs: number, nowMs: number): string {
  const sec = Math.max(0, Math.floor((nowMs - fetchedAtMs) / 1000));
  if (sec < 30) return "обновлено только что";
  if (sec < 3600) return `обновлено ${Math.floor(sec / 60)} мин назад`;
  return `обновлено ${Math.floor(sec / 3600)} ч назад`;
}

/** Черновики уходят вниз, остальное — по свежести. Копия, не мутация. */
export function sortPrs(items: PrItem[]): PrItem[] {
...
- [ ] **Step 5: Бэкенд-команды**

В `src-tauri/src/commands.rs`:

```rust
/// Общая часть: воркспейс → путь + окружение его аккаунта.
fn workspace_ctx(state: &State<AppState>, workspace_id: &str) -> Result<(String, Vec<(String, String)>), String> {
    let ws = state
        .store.lock().unwrap().workspaces().into_iter()
        .find(|w| w.id == workspace_id)
        .ok_or_else(|| "воркспейс не найден".to_string())?;
    let noauth_dir = state.store.lock().unwrap().dir.join("gh-noauth");
    let _ = std::fs::create_dir_all(&noauth_dir);
    let outcome = resolve_session_auth(
        ws.github.as_ref(),
    ...
    Ok((ws.path, outcome.env))
}

#[tauri::command]
pub fn gh_repo(state: State<AppState>, workspace_id: String) -> Result<Option<String>, String> {
    let (cwd, env) = workspace_ctx(&state, &workspace_id)?;
    Ok(gh::repo(&cwd, &env))
}

#[tauri::command]
pub fn gh_pull_requests(state: State<AppState>, workspace_id: String) -> Result<Vec<gh::PrItem>, String> {
    let (cwd, env) = workspace_ctx(&state, &workspace_id)?;
    gh::pull_requests(&cwd, &env)
}

#[tauri::command]
pub fn gh_issues(state: State<AppState>, workspace_id: String) -> Result<Vec<gh::IssueItem>, String> {
    let (cwd, env) = workspace_ctx(&state, &workspace_id)?;
    gh::issues(&cwd, &env)
}
```

Зарегистрировать три команды в `invoke_handler` в `src-tauri/src/main.rs`.
...
```bash
git add src-tauri/src/commands.rs src-tauri/src/main.rs src/ipc.ts src/github.ts tests/github.test.ts
git commit -m "feat(github): PR and issue commands with an on-demand frontend cache"
```

---

### Task 16: Счётчики в сайдбаре и экран списков

**Files:**
- Modify: `src/workspaces.ts` (счётчики), `src/github-screen.ts` (экран списков), `src/styles.css`

**Interfaces:**
- Consumes: `ghRepo`, `ghPullRequests`, `ghIssues` (Task 15), `sortPrs`, `relativeAge`, `isStale` (Task 15)
- Produces: `openWorkspaceItemsScreen(workspaceId: string, workspaceName: string): Promise<void>`

- [ ] **Step 1: Экран списков**

В `src/github-screen.ts` добавить экспорт: оверлей в том же стиле, две секции — «Пулреквесты» и «Задачи». Каждая строка: `#номер`, заголовок, автор, для PR — пометка черновика и решение ревью, ссылка «открыть» (`<a target="_blank" rel="noreferrer">`). Под заголовком — строка `relativeAge(...)` и кнопка «Обновить». При ошибке показать её текст и, если кеш есть, отрисовать его с пометкой возраста.

Кеш — модульная `Map<string, { prs: PrItem[]; issues: IssueItem[]; fetchedAt: number }>`, чтение через `isStale`.

- [ ] **Step 2: Счётчики у строки воркспейса**

В `src/workspaces.ts` в `render()` для воркспейсов с `github` добавить кликабельный элемент `PR n · issues m`, открывающий экран. Данные тянуть лениво — при первом рендере после выбора воркспейса и по возврату с экрана, не при каждом `render()`.

```ts
      if (w.github) {
        const counts = document.createElement("button");
        counts.className = "ws-counts";
        counts.textContent = "PR — · issues —";
        counts.onclick = () => void openWorkspaceItemsScreen(w.id, w.name);
        row.append(counts);
      }
```

- [ ] **Step 3: Крайние случаи**

- Воркспейс не git-репозиторий или без remote → `ghRepo` вернёт `null`: счётчики не показываем, экран пишет «репозиторий не определён».
- Ошибка `gh` (нет прав, рейт-лимит, оффлайн) → показываем текст ошибки как есть; если кеш непустой, рисуем его с пометкой возраста.
- Пустые списки → «открытых пулреквестов нет» / «открытых задач нет».

- [ ] **Step 4: Стили**

В `src/styles.css` добавить `.ws-counts`, `.gh-item-row`, `.gh-item-num`, `.gh-item-meta`, `.gh-item-draft` на существующих токенах.
...
```bash
git add src/workspaces.ts src/github-screen.ts src/styles.css
git commit -m "feat(github): workspace PR/issue counters and list screen"
```

---

### Task 17: Документация фазы 2 и ручная проверка изоляции
...
```markdown
- **PR и задачи воркспейса** — счётчик открытых пулреквестов и issues у строки воркспейса и экран со списками, всё под аккаунтом этого воркспейса. Только чтение: действия совершает сессия.
```

В раздел про аккаунты добавить абзац о том, что списки обновляются по требованию (при открытии экрана и по кнопке), фонового опроса GitHub нет.

- [ ] **Step 2: Ручная проверка изоляции**

Автотестами это не закрыть, поэтому прогнать руками и записать результат в PR:

1. Два воркспейса, привязанных к разным аккаунтам.
2. Запустить по сессии в каждом **одновременно**.
3. В каждой выполнить `gh auth status` — показывает свой аккаунт.
4. `gh pr list` в обеих — результаты разных аккаунтов.
5. В одной попробовать `gh auth switch` — gh отказывает, вторая сессия не затронута.
6. Проверить, что вне приложения (в своём терминале) `gh auth status` показывает прежний активный аккаунт — приложение его не меняло.
...
```bash
git add README.md
git commit -m "docs: workspace PR and issue lists"
```
...
**Покрытие спеки.** Каждый раздел спеки закрыт: модель данных → Task 1; `gh.rs` (детект, аккаунты, токен, env) → Tasks 2–4; `pty.rs` → Task 5; резолв в `start_session` → Task 6; команды `gh_status`/`host_platform` → Task 7; `github.ts` → Task 8; установка и логин через тайл → Tasks 9–10; форма воркспейса → Task 11; индикация → Task 12; фаза 2 (`repo`/`pull_requests`/`issues`, кеш, счётчики, экран) → Tasks 14–16; README → Tasks 13, 17. Инвариант «токен не утекает» реализуется в Task 4 (`redact`) и проверяется в Tasks 3 и 6.

**Согласованность типов.** `WorkspaceGithub` (Task 1) используется в Tasks 3, 6, 11 с теми же именами полей; `SessionAuth` (Task 6) — в Task 12; `GhStatus`/`GhAccount` (Tasks 2, 7) — в Tasks 8, 10, 11; `PrItem`/`IssueItem` (Task 14) — в Tasks 15, 16. Сигнатура `session_env(cfg, token, noauth_dir)` одинакова в Tasks 3, 6, 15.

**Плейсхолдеры.** Проверено: «TBD», «добавить обработку ошибок», «аналогично Task N» и шагов без кода в плане нет. Единственные места, где вместо кода дано словесное описание, — вёрстка (Task 16 шаги 1–2 и стили в Tasks 10, 12, 16): там перечислены конкретные классы, содержимое строк и крайние случаи, а точная разметка отдана на усмотрение реализующего, поскольку тестами она не покрывается и должна лечь в существующую сетку `styles.css`.

**Порядок исполнения.** Tasks 1–13 — фаза 1, самодостаточна и может быть смёржена отдельно. Tasks 14–17 опираются на Task 6 (`resolve_session_auth`) и Task 3 (`session_env`), поэтому идут строго после фазы 1.
