...
- Produces:
  ...
  - `pub fn load(cache: &Path) -> Index` — a missing, corrupt, stale or length-mismatched cache returns an empty index rather than failing.
  - `pub fn save(cache: &Path, ix: &Index) -> anyhow::Result<()>` — refuses to write an index that violates its own invariant.
...
**Two integrity rules, both learned from review.** `save` writes two files, so a crash between them can pair a new `meta.json` with a stale `emb.bin`. If the chunk *count* happens to match — an edit that re-chunks into the same number of pieces, or an update that drops one chunk and adds another — a length check alone passes and `load` hands back vectors that do not belong to their texts. That is silently wrong search results, which is strictly worse than losing the cache, and it contradicts the contract that a corrupt cache behaves like a missing one.

So `emb.bin` carries an 8-byte little-endian fingerprint of the exact chunk list it was computed for, and `load` recomputes and compares. Both files are written to temporaries and renamed, so a truncated file is never visible.

Separately, `dim == 0` makes `chunks.len() * dim == 0` and the length check vacuous. The storage layer enforces its own invariant: `save` refuses an inconsistent index, and `load` rejects `dim == 0` alongside a non-empty chunk list.
