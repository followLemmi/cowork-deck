...
- Each column with a `data-step` gets `dragover` calling `preventDefault` (without it there is no drop) and toggling `tk-col-over`, and `drop` reading the id, finding the task, and calling `onMove` unless the step is the one it already has — **or unless the task it just looked up is damaged or conflicting.**

  That second condition is not redundant with withholding `draggable`. `draggable` is decided when the card is *rendered*; `drop` looks the task up in the freshest `state.tasks`, which is deliberate and right — a drop should act on the card as it is now, not as it was when the drag began. The gap between those two moments is real on a board that re-polls every five seconds: a card that was writable at `dragstart` can be damaged by the time the drop lands, and only the drop-side check sees that. Withholding the affordance is not the same as refusing the write, which is exactly the distinction the modal draws by disabling Save itself rather than only the fields. Test it by dropping a card the state reports as damaged and asserting `onMove` is never called.
...
**The provider's board must be able to answer about both ends of the move.** Building it from the draft alone is not enough, and the two fixes above collide if you stop there. `update` asks its board two different questions: *which ids may I write* — correctly the draft — and *was the card's current step terminal*, which needs the configuration the card is being renamed **out of**. A renamed-away id is by definition absent from the draft, so `is_terminal(from)` answers `false` for every card a rewrite touches, whatever that step actually was. Renaming `done` → `shipped` then reads as `(false, true)` and restamps every card, and retiring a terminal step reads as `(false, false)` and preserves a `resolved` that should have been cleared. Only a rewrite whose `from` was already non-terminal gets the right answer, by luck.

So `rewrite_step` builds its board as **the draft, plus the `from` step as the on-disk configuration defines it** when the draft does not list it. Then both endpoints are answerable: `to` from the draft, `from` with the terminality it actually had. Test it at the rewrite level, not only through `update` — a terminal-to-terminal rename must preserve `resolved` byte for byte, and retiring a terminal step to a non-terminal destination must clear it. Neither is covered by a test that drives `update` directly on one provider whose board never drops either step.
